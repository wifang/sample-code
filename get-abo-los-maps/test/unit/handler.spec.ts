...
import { config, repo, aboLosMapGet } from '../../src/handler';
import { LosMapSetRepository } from '../../src/losMap-set.repository';
import * as successEvent from '../local_200_mock.json';
import * as invalidPeriod from '../local_invalidperiod_mock.json';
import nock from 'nock';

describe("exports config service", () => {
    it('exports returns config service', () => {
        const service: ConfigService = config();
        expect(service).to.be.not.null;
    });
});


describe("exports los map set repository", () => {
    const configService: ConfigService = config();
    it('exports los map set repository, dev, EUR', () => {
        const repository: LosMapSetRepository = repo(configService, 'dev', 'EUR');
        expect(repository).to.be.not.null;
    });
    it('exports los map set repository, dev, ANA', () => {
        const repository: LosMapSetRepository = repo(configService, 'dev', 'ANA');
        expect(repository).to.be.not.null;
    });
    
    it('exports los map set repository dev, global', () => {
        const repository: LosMapSetRepository = repo(configService,'dev', 'global');
        expect(repository).to.be.not.null;
    });
    it('exports los map set repository, dev, default', () => {
        const repository: LosMapSetRepository = repo(configService, 'dev');
        expect(repository).to.be.not.null;
    });
    it('exports los map set repository, qa, EUR', () => {
        const repository: LosMapSetRepository = repo(configService, 'qa', 'EUR');
        expect(repository).to.be.not.null;
    });
    it('exports los map set repository, qa, ANA', () => {
        const repository: LosMapSetRepository = repo(configService, 'qa', 'ANA');
        expect(repository).to.be.not.null;
    });
    it('exports los map set repository qa, global', () => {
        const repository: LosMapSetRepository = repo(configService,  'qa', 'global');
        expect(repository).to.be.not.null;
    });
    it('exports los map set repository, qa, default', () => {
        const repository: LosMapSetRepository = repo(configService,  'qa');
        expect(repository).to.be.not.null;
    });
    it('exports los map set repository, test, EUR', () => {
        const repository: LosMapSetRepository = repo(configService, 'test', 'EUR');
        expect(repository).to.be.not.null;
    });
    it('exports los map set repository, test, ANA', () => {
        const repository: LosMapSetRepository = repo(configService, 'test', 'ANA');
        expect(repository).to.be.not.null;
    });
    it('exports los map set repository test, global', () => {
        const repository: LosMapSetRepository = repo(configService,'test', 'global');
        expect(repository).to.be.not.null;
    });
    it('exports los map set repository, test, default', () => {
        const repository: LosMapSetRepository = repo(configService, 'test');
        expect(repository).to.be.not.null;
    });
    it('exports los map set repository, prod, EUR', () => {
        const repository: LosMapSetRepository = repo(configService, 'prod', 'EUR');
        expect(repository).to.be.not.null;
    });
    it('exports los map set repository, prod, ANA', () => {
        const repository: LosMapSetRepository = repo(configService, 'prod', 'ANA');
        expect(repository).to.be.not.null;
    });
    it('exports los map set repository prod, global', () => {
        const repository: LosMapSetRepository = repo(configService, 'prod', 'global');
        expect(repository).to.be.not.null;
    });
    it('exports los map set repository, prod, default', () => {
        const repository: LosMapSetRepository = repo(configService, 'prod');
        expect(repository).to.be.not.null;
    });
});

describe('handler general case', () => {
    const successBody = {
        "volumeDetailPeriods": [
            {
                "aff": 180,
    ...
    beforeEach(() => {
        nock('https://api-dv.amwayglobal.com/rest/v1/losv3/magicLosApp/details/')
            .get('/?affAbo[]=180-100038&period[]=201912&includeMap=true&includeAboAttr=true')
            .reply(200, successBody);
    });

    it('abo with all los map types defined', async() => {
        const response = await aboLosMapGet(successEvent, undefined, undefined);
        expect(response).to.be.not.null;
        expect(response.statusCode).to.equal(200);
        const body = JSON.parse(response.body);
        expect(body).to.be.not.null;
        expect(body.losMap).to.be.not.null;
    });

});

describe('handler period test', () => {
    const invalidPeriodReponse = {
        "status": 400,
    ...
    beforeEach(() => {
        nock('https://api-dv.amwayglobal.com/rest/v1/losv3/magicLosApp/details/')
        .get('/?affAbo[]=180-100038&period[]=201913&includeMap=true&includeAboAttr=true')
        .reply(400, invalidPeriodReponse);
    });

    it('invalid period for valid abo', async() => {
        const response = await aboLosMapGet(invalidPeriod, undefined, undefined);
        expect(response.statusCode).to.equal(400);
    });
});
describe('handler, magic down test', () => {
    const systemDown = `<!DOCTYPE html>
    <html>
        <head>
    ...
    beforeEach(() => {
        nock('https://api-dv.amwayglobal.com/rest/v1/losv3/magicLosApp/details/')
        .get('/?affAbo[]=180-100038&period[]=201912&includeMap=true&includeAboAttr=true')
            .replyWithError(systemDown);
    });

    it('source system down', async() => {
        const response = await aboLosMapGet(successEvent, undefined, undefined);
        expect(response.statusCode).to.equal(500);
        const body = JSON.parse(response.body); 
        expect(body.message).to.equal('Server Error in MAGIC DMS');
